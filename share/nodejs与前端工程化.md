...
**状态**
- 用户管理
    - project code 版本控制下的源码
    - manifest file 依赖记录文件
- 机器管理
    - lock file 包含生成完整依赖性源树所需的所有信息
    - dependency code 依赖包的代码

**协议**
- 线性关系
    - 项目代码 -> manifest文件 -> lock file文件 -> 依赖包代码
    - 编写引用包识  ->  根据引用情况CURD，用命令行工具进行装载 -> 根据依赖情况记录版本等信息 -> 根据记录情况进行下载和存储 
- 根据commit历史进行依赖区别
- 每一次commit产生的源码得到不同的代码情况再交付给编译器
### 8.3 npm如何解决不同版本的包引用
假如A->C1.0 B->C1.1
1. 分析A是否能转化到C1.1或者B是否能转化到C1.0，如果不能则到2
2. 分析是否有一个C的版本能同时满足AB，不能则到3
3. 自己创建一个C的版本同时满足AB
